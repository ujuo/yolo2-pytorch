...
def _process_batch(data, size_index):
   ...
   ## print("W: ",W, "H: ",H, "inp_size: ",inp_size, "out_size: ",out_size)
    bbox_pred_np, gt_boxes, gt_classes, dontcares, iou_pred_np = data

#    print("bbox_pred_np: ", bbox_pred_np, "gt_boxes: ",gt_boxes, 
#        "gt_classes: ", gt_classes, "iou_pred_np: ", iou_pred_np)
    # net output
    hw, num_anchors, _ = bbox_pred_np.shape
#    print("hw: ", hw, "num_anchors", num_anchors)
    ...
    ##print("bbox_pred_np shape : ", bbox_pred_np.shape) #, "bbox_pred_np : ", bbox_pred_np)
    ##print("bbox_pred_np data: ", bbox_pred_np[0])
    bbox_pred_np = np.expand_dims(bbox_pred_np, 0)
    ##print("anchor shape : ", anchors.shape) #"anchors: ", anchors)
    ##print("bbox_pred_np expand_dims shape : ", bbox_pred_np.shape) #, "bbox_pred_np: ", bbox_pred_np)
    #print("bbox_pred_np exp data: ", bbox_pred_np[0])
    #x = np.ascontiguousarray(bbox_pred_np, dtype=np.float)
   # print("x data: ", x[0])
...
class YoloLoss(nn.Module):
    ...
    def forward(self, out,gt_boxes=None, gt_classes=None, dontcare=None,
                size_index=0,training=True):


        self.training = training

        bsize, _, h, w = out.size()
        global_average_pool_reshaped =out.permute(0,2,3,1).contiguous().view(bsize, -1,
                                                                   cfg.num_anchors, cfg.num_classes+5)

        # for detection
        # bsize, c, h, w -> bsize, h, w, c ->
        #                   bsize, h x w, num_anchors, 5+num_classes
        ##bsize, _, h, w = global_average_pool.size() #b,c,h,w
        ...
        bbox_pred = torch.cat([xy_pred, wh_pred], 3) #xy_pred + wh_pred

        score_pred = global_average_pool_reshaped[:, :, :, 5:].contiguous()

       # prob_pred= F.softmax(global_average_pool_reshaped, dim=-1)
